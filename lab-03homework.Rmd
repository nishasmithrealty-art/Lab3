...
hvi_tidy <- hvi_tidy %>%
  arrange(StateName, Region) %>%             
  group_by(StateName) %>%         
  
  mutate(HVI = na.approx(HVI,x = Date, na.rm = FALSE))%>% ungroup()
hvi_tidy
...
# TODO
ggplot(data = hvi_tidy_NA_remove %>%
         filter(Date == as.Date("2025-8-31")), aes(x = Popu2024)) +
  geom_dotplot(
    alpha = 0.9, method = "histodot", color = "2E86AB", binwidth = 300000
        
  ) +
  labs(title = "Dotplot of Metro Stata Area Population", x = "Population -2024 ", y = "City Numbers") +
  theme_minimal()
...
ggplot(
  ...
  labs(
    title = "Boxplot Census Bureau Regions city Populaion",
    y = "Population - 2024",
    x = NULL
  ) +
  theme_minimal()
...
#### 📚 NMDB: Outstanding Residential Mortgage Statistics ----
library(tidyverse)
library(skimr)
library(ggplot2)


nmdb <- read_csv("C:/Users/nisha/OneDrive/Desktop/Class R/lab-03/nmdb-new-mortgage-statistics-national-census-areas-monthly.csv")
...
metrics <- c("TOT_ORIG", "PCT_HP", "AVE_PROPVAL")
nmdb_recent <- nmdb_state %>%
  filter(SERIESID %in% metrics,
         YEAR == max(YEAR, na.rm = TRUE))

#Summary statistics: WA vs US
wa_stats <- nmdb_recent %>%
...
# Wide Format
nmdb_wide <- nmdb_recent %>%
  select(GEONAME, YEAR, QUARTER, SERIESID, VALUE1) %>%
...
# 1. Histogram
ggplot(filter(nmdb_recent, SERIESID == "TOT_ORIG"),
       aes(x = VALUE1)) +
  geom_histogram(bins = 20, fill = "#4B2E83", color = "white") +
  labs(title = "Distribution of Total Loans by State",
       x = "Total Loans", y = "Count of States") +
  theme_minimal()

# 2. Boxplot
ggplot(nmdb_recent %>%
         filter(SERIESID == "TOT_ORIG") %>%
         mutate(Group = if_else(GEONAME == "Washington", "Washington", "Other States")),
       aes(x = Group, y = VALUE1)) +
  geom_boxplot(fill = "gray") +
  geom_jitter(width = 0.2, alpha = 0.6, color = "#D72638") +
  theme_minimal()
